...
impl Generator for KotlinNativeGenerator {
    fn generate(&self, manifest: &ScriptManifest) -> anyhow::Result<String> {
        ...
        writeln!(output, "@file:OptIn(ExperimentalForeignApi::class, ExperimentalNativeApi::class)")?;
        writeln!(output)?;
        ...
        writeln!(output, "private val scriptInstances: List<ScriptRegistration> by lazy {{")?;
        ...
        writeln!(output, "    override fun getScripts(): List<ScriptRegistration> {{")?;
        ...
        for (i, item) in manifest.items().iter().enumerate() {
            ...
            let tags_str = if item.tags().is_empty() {
                ...
                let tag_list: Vec<String> = item.tags()
                    .iter()
                    .map(|tag| format!("\"{}\"", tag))
                    .collect();
        ...
        writeln!(output, "fun getDropbearEngine(worldPointer: COpaquePointer?, currentEntity: Long?): DropbearEngine {{")?;
        writeln!(output, "    val nativeEngine = NativeEngine()")?;
        writeln!(output, "    nativeEngine.init(worldPointer)")?;
        writeln!(output, "    val dropbearEngine = DropbearEngine(nativeEngine, if (currentEntity == null) null else EntityRef(EntityId(currentEntity)))")?;
        ...
        writeln!(output, "fun loadScriptByTag(worldPointer: COpaquePointer?, currentEntity: Long?, tag: String?) {{")?;
        writeln!(output, "    if (tag == null) return")?;
        writeln!(output, "    val scripts = scriptInstances.filter {{ it.tags.contains(tag) }}")?;
        writeln!(output, "    val engine = getDropbearEngine(worldPointer, currentEntity)")?;
        ...
        writeln!(output, "fun updateScriptByTag(worldPointer: COpaquePointer?, currentEntity: Long?, tag: String?, deltaTime: Double) {{")?;
        writeln!(output, "    if (tag == null) return")?;
        writeln!(output, "    val scripts = scriptInstances.filter {{ it.tags.contains(tag) }}")?;
        writeln!(output, "    val engine = getDropbearEngine(worldPointer, currentEntity)")?;
        writeln!(output, "    for (script in scripts) {{")?;
        writeln!(output, "        script.script.update(engine, deltaTime.toFloat())")?;
        writeln!(output, "    }}")?;
        writeln!(output, "}}")?;
        writeln!(output)?;
        ...
        writeln!(output, "fun destroyScriptByTag(worldPointer: COpaquePointer?, currentEntity: Long?, tag: String?) {{")?;
        writeln!(output, "    if (tag == null) return")?;
        writeln!(output, "    val scripts = scriptInstances.filter {{ it.tags.contains(tag) }}")?;
        writeln!(output, "    val engine = getDropbearEngine(worldPointer, currentEntity)")?;
        writeln!(output, "    for (script in scripts) {{")?;
